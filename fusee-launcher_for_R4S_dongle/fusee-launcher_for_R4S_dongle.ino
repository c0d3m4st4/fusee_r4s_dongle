...
// From what I can tell, usb.outTransfer is completely broken for transfers larger than 64 bytes (even if maxPktSize is adjusted for that endpoint). This is a minimal and simplified reimplementation specific to 
// our use cases that fixes that bug and is based on the code of USBHost::outTransfer, USBHost::SetPipeAddress and USBHost::OutTransfer.
...
// This accepts arbitrary sized USB writes and will automatically chunk them into writes of size 0x1000 and increment packetsWritten every time a chunk is written out.
...
void readTegraDeviceID(byte *deviceID)
{
    ...
    if (usb.inTransfer(tegraDeviceAddress, 0x01, &readLength, deviceID))
        fail = true; // Failed to get device ID
}
...
{  
    ...
    usb.ctrlReq(tegraDeviceAddress, 0, USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_STANDARD | USB_SETUP_RECIPIENT_INTERFACE, 0x00, 0x00, 0x00, 0x00, 0x7000, 0x7000, usbWriteBuffer, NULL);
    
    // Done

    // Turn off LED and go into deep sleep mode
    digitalWrite(led,LOW);
